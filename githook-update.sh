...
endmsg
    ...
    branch=`git status`
    if [[ "$branch" == "On branch master"* ]]; then
      echo "\n\n\033[1;31m[WARNING] You are committing to the \033[1;36mmaster\033[1;31m branch, please switch to a feature branch\033[0m\n"
      exit 1;
    fi
endmsg
    ...
    # checkout github gmaster
    # pull down current github master branch on gmaster branch
    # checkout origin master
    # pull down current origin master branch
    # merge into github into origin
    # push origin master
    ;;
    *)
    echo "Usage: $0 {update|deploy}"
    exit 1
    ;;
esac
